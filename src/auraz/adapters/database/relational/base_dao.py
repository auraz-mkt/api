...
class BaseDAO(Repository[EntityType]):
    ...
    async def create(self, **kwargs) -> Model[EntityType]:
        new_item = self.__build_item_from_scratch(**kwargs)
    ...
    async def update(self, searched_id: ID, **kwargs) -> Model[EntityType]:
        ...
        updated_item = self.__build_item_from_prototype(found_item, **kwargs)

        return self.__save(updated_item)

    def __build_item_from_scratch(self, **kwargs) -> Model[EntityType]:
        assert self.__check_fields_exist(set(kwargs.keys()))
        return self.model_type(**kwargs)

    def __build_item_from_prototype(self, original_item: Model[EntityType], **kwargs) -> Model[EntityType]:
        assert self.__check_fields_exist(set(kwargs.keys()))
